...
async function emptyAccountExportFetch(
...
afterEach(async () => {
  ...
  delete process.env.WATCHBRIDGE_API_KEYS;
  delete process.env.WATCHBRIDGE_PORT;
  delete process.env.WATCHBRIDGE_SHUTDOWN_TIMEOUT_MS;
  delete process.env.WATCHBRIDGE_HEADERS_TIMEOUT_MS;
  delete process.env.WATCHBRIDGE_REQUEST_TIMEOUT_MS;
  delete process.env.WATCHBRIDGE_KEEP_ALIVE_TIMEOUT_MS;
  delete process.env.WATCHBRIDGE_RATE_LIMIT_PER_MINUTE;
...
describe("API health and readiness", () => {
  it("keeps liveness separate from configuration and persistent-storage readiness", async () => {
    const backupDirectory = await mkdtemp(
      join(tmpdir(), "watchbridge-ready-backups-"),
    );
    const jobDirectory = await mkdtemp(
      join(tmpdir(), "watchbridge-ready-jobs-"),
    );
    const vaultDirectory = await mkdtemp(
      join(tmpdir(), "watchbridge-ready-vault-"),
    );
    const transactionDirectory = await mkdtemp(
      join(tmpdir(), "watchbridge-ready-transactions-"),
    );
    temporaryBackupDirectories.push(
      backupDirectory,
      jobDirectory,
      vaultDirectory,
    ...
    process.env.WATCHBRIDGE_OAUTH_VAULT_DIR = vaultDirectory;
    process.env.WATCHBRIDGE_OAUTH_TRANSACTION_DIR = transactionDirectory;
    process.env.WATCHBRIDGE_STORAGE_KEY = "01".repeat(32);

    expect(app.request("/healthz")).toMatchObject({ status: 200 });
    const ready = await app.request("/readyz");
    expect(ready.status).toBe(200);
    await expect(ready.json()).resolves.toEqual({
      ok: true,
      service: "watchbridge-api",
    });
    expect(await readdir(vaultDirectory)).toEqual([]);

    const vaultFile = join(vaultDirectory, "not-a-directory");
    await writeFile(vaultFile, "not a directory", "utf8");
    process.env.WATCHBRIDGE_OAUTH_VAULT_DIR = vaultFile;
    expect((await app.request("/readyz")).status).toBe(503);
    expect(app.request("/healthz")).toMatchObject({ status: 200 });

    await rm(vaultFile);
    process.env.WATCHBRIDGE_OAUTH_VAULT_DIR = vaultDirectory;
    const transactionFile = join(transactionDirectory, "not-a-directory");
    await writeFile(transactionFile, "not a directory", "utf8");
    process.env.WATCHBRIDGE_OAUTH_TRANSACTION_DIR = transactionFile;
    expect((await app.request("/readyz")).status).toBe(503);

    process.env.WATCHBRIDGE_PORT = "70000";
    const unready = await app.request("/readyz");
    expect(unready.status).toBe(503);
    await expect(unready.json()).resolves.toEqual({
      ok: false,
      service: "watchbridge-api",
    });
  });

  it("requires a suitably sized API key for production readiness", async () => {
    const previousNodeEnv = process.env.NODE_ENV;
    const backupDirectory = await mkdtemp(
      join(tmpdir(), "watchbridge-ready-production-backups-"),
    );
    const jobDirectory = await mkdtemp(
      join(tmpdir(), "watchbridge-ready-production-jobs-"),
    );
    const vaultDirectory = await mkdtemp(
      join(tmpdir(), "watchbridge-ready-production-vault-"),
    );
    const transactionDirectory = await mkdtemp(
      join(tmpdir(), "watchbridge-ready-production-transactions-"),
    );
    temporaryBackupDirectories.push(
      backupDirectory,
      jobDirectory,
  ...
  it("validates the bounded graceful-shutdown timeout", async () => {
    expect(apiShutdownTimeout()).toBe(25_000);
    expect(apiShutdownTimeout("1000")).toBe(1_000);
    expect(apiShutdownTimeout("300000")).toBe(300_000);
    expect(() => apiShutdownTimeout("999")).toThrow(
      "WATCHBRIDGE_SHUTDOWN_TIMEOUT_MS",
    );
    expect(() => apiShutdownTimeout("300001")).toThrow(
      "WATCHBRIDGE_SHUTDOWN_TIMEOUT_MS",
    );
    expect(() => apiShutdownTimeout("15s")).toThrow(
      "WATCHBRIDGE_SHUTDOWN_TIMEOUT_MS",
    );
  });

  it("bounds transport timeouts and applies them to the Node server", () => {
    expect(apiTransportTimeouts()).toEqual({
      headersTimeout: 15_000,
      requestTimeout: 60_000,
      keepAliveTimeout: 5_000,
    });
    expect(apiTransportTimeouts("12000", "90000", "4000")).toEqual({
      headersTimeout: 12_000,
      requestTimeout: 90_000,
      keepAliveTimeout: 4_000,
    });
    expect(() => apiTransportTimeouts("90001", "90000")).toThrow(
      "must not exceed",
    );
    expect(() => apiTransportTimeouts("slow")).toThrow(
      "WATCHBRIDGE_HEADERS_TIMEOUT_MS",
    );
    expect(() => apiTransportTimeouts(undefined, "999")).toThrow(
    ...
    configureTransportTimeouts(server, {
      headersTimeout: 12_000,
      requestTimeout: 90_000,
      keepAliveTimeout: 4_000,
    });
  ...
  it("drains once, exits cleanly, and forces exit only after the shutdown deadline", () => {
    ...
    const shutdown = createGracefulShutdown(server, 1_000, exit, logger);
  ...
  it("forces a non-zero exit when active requests do not drain before the deadline", () => {
    vi.useFakeTimers();
    const server: ShutdownServer = {
      close: vi.fn(),
      closeAllConnections: vi.fn(),
    ...
    const exit = vi.fn();
    const shutdown = createGracefulShutdown(server, 1_000, exit, {
      log: vi.fn(),
      error: vi.fn(),
    });
    shutdown("SIGTERM");
  ...
  it("applies a bounded per-process production API rate limit", async () => {
    const previousNodeEnv = process.env.NODE_ENV;
    process.env.NODE_ENV = "production";
    process.env.WATCHBRIDGE_API_KEY = "r".repeat(32);
    process.env.WATCHBRIDGE_RATE_LIMIT_PER_MINUTE = "1";

    try {
      const headers = {
        Authorization: `Bearer ${process.env.WATCHBRIDGE_API_KEY}`,
      };
      expect((await app.request("/v1/services", { headers })).status).toBe(200);
      const limited = await app.request("/v1/services", { headers });
  ...
  it("bounds concurrent production sync executions before routes can create a job", async () => {
    const previousNodeEnv = process.env.NODE_ENV;
    process.env.NODE_ENV = "production";
    process.env.WATCHBRIDGE_API_KEY = "s".repeat(32);
    process.env.WATCHBRIDGE_MAX_CONCURRENT_SYNCS = "1";
    const release = acquireSyncExecutionSlot();

    try {
      expect(apiMaxConcurrentSyncs()).toBe(1);
      expect(release).toBeTypeOf("function");
      expect(acquireSyncExecutionSlot()).toBeUndefined();
      ...
      const metrics = await app.request("/v1/metrics", {
        headers: { Authorization: `Bearer ${process.env.WATCHBRIDGE_API_KEY}` },
      });
      const metricsText = await metrics.text();
      expect(metricsText).toContain("watchbridge_sync_executions_active 1");
      expect(metricsText).toContain("watchbridge_sync_executions_limit 1");
    } finally {
      release?.();
      process.env.NODE_ENV = previousNodeEnv;
    }
  ...
  it("bounds concurrent authenticated production API requests before reading request bodies", async () => {
    const previousNodeEnv = process.env.NODE_ENV;
    process.env.NODE_ENV = "production";
    process.env.WATCHBRIDGE_API_KEY = "q".repeat(32);
    expect(apiMaxConcurrentRequests()).toBe(16);
    expect(apiMaxConcurrentRequests("1")).toBe(1);
    expect(apiMaxConcurrentRequests("200")).toBe(200);
    expect(() => apiMaxConcurrentRequests("0")).toThrow(
      "WATCHBRIDGE_MAX_CONCURRENT_REQUESTS",
    );
    expect(() => apiMaxConcurrentRequests("201")).toThrow(
      "WATCHBRIDGE_MAX_CONCURRENT_REQUESTS",
    );
    expect(() => apiMaxConcurrentRequests("sixteen")).toThrow(
      "WATCHBRIDGE_MAX_CONCURRENT_REQUESTS",
    );
    process.env.WATCHBRIDGE_MAX_CONCURRENT_REQUESTS = "1";
    const release = acquireApiRequestSlot();

    try {
      expect(apiMaxConcurrentRequests()).toBe(1);
      expect(release).toBeTypeOf("function");
      expect(acquireApiRequestSlot()).toBeUndefined();
      const response = await app.request("/v1/services", {
        headers: { Authorization: `Bearer ${process.env.WATCHBRIDGE_API_KEY}` },
      });
      expect(response.status).toBe(429);
      ...
      const metricsText = await metrics.text();
      expect(metricsText).toContain("watchbridge_api_requests_active 1");
      expect(metricsText).toContain("watchbridge_api_requests_limit 1");
    } finally {
      release?.();
      process.env.NODE_ENV = previousNodeEnv;
    }
...
describe("storage retention cleanup", () => {
  it("previews and deletes expired records while preserving pending jobs and referenced backups", async () => {
    const backupDirectory = await mkdtemp(
      join(tmpdir(), "watchbridge-retention-backups-"),
    );
    const jobDirectory = await mkdtemp(
      join(tmpdir(), "watchbridge-retention-jobs-"),
    );
    ...
    await writeFile(
      join(jobDirectory, `${pendingJobId}.json`),
      JSON.stringify(
        job(pendingJobId, "pending", oldTimestamp, referencedBackupId),
      ),
    );
    await writeFile(
      join(jobDirectory, `${expiredJobId}.json`),
      JSON.stringify(
        job(expiredJobId, "succeeded", oldTimestamp, orphanBackupId),
      ),
    );
    ...
    await utimes(
      join(backupDirectory, `${referencedBackupId}.json`),
      oldDate,
      oldDate,
    );
    await utimes(
      join(backupDirectory, `${orphanBackupId}.json`),
      oldDate,
      oldDate,
    );
    ...
    expect((await readdir(backupDirectory)).sort()).toEqual(
      [`${referencedBackupId}.json`, `${freshBackupId}.json`].sort(),
    );
  });
  ...
  it("fails closed for invalid retention configuration and unknown request fields", async () => {
    ...
    await expect(invalid.json()).resolves.toEqual({
      error:
        "WATCHBRIDGE_BACKUP_RETENTION_DAYS must be 0 or a whole number of days.",
    });
  ...
  it("blocks backup deletion when the job inventory cannot be trusted", async () => {
    const backupDirectory = await mkdtemp(
      join(tmpdir(), "watchbridge-retention-guard-backups-"),
    );
    const jobDirectory = await mkdtemp(
      join(tmpdir(), "watchbridge-retention-guard-jobs-"),
    );
    temporaryBackupDirectories.push(backupDirectory, jobDirectory);
    process.env.WATCHBRIDGE_BACKUP_DIR = backupDirectory;
    process.env.WATCHBRIDGE_JOB_DIR = jobDirectory;
  ...
  it("applies configured job retention automatically before creating a new audit job", async () => {
    const jobDirectory = await mkdtemp(
      join(tmpdir(), "watchbridge-retention-automatic-jobs-"),
    );
...
describe("shared sync-job locking", () => {
  it("serializes concurrent owners and reclaims a stale owner lock", async () => {
    ...
    await expect(withJobLock(id, async () => "reclaimed")).resolves.toBe(
      "reclaimed",
    );
...
describe("mapped CSV import endpoint", () => {
  ...
  it("rejects incomplete import mappings", async () => {
    const response = await app.request("/v1/import/mapped-csv", {
      ...
      body: JSON.stringify({
        csv: "x",
        config: { service: "serializd", columns: {} },
      }),
    });
    ...
    await expect(missingScale.json()).resolves.toMatchObject({
      error: expect.stringContaining("ratingScale"),
    });
  });
  ...
  it("keeps the mapped-CSV endpoint scoped to registry manual workflows", async () => {
    ...
    expect(response.status).toBe(400);
    await expect(response.json()).resolves.toMatchObject({
      error: expect.stringContaining("manual-mapping"),
    });
  });
...
describe("provider file import endpoint", () => {
  it("returns a strict executable backup-v1 archive at the top level", async () => {
    const response = await app.request("/v1/import/provider-files", {
      ...
      body: JSON.stringify({
        ...
        files: {
          ratings:
            "Const,YourRating,DateRated,Title,TitleType,Year\ntt0113277,9,2026-01-01,Heat,movie,1995",
          watchlist:
  ...
  it("returns sanitized 400 responses for strict-shape and content failures", async () => {
    const unknown = await app.request("/v1/import/provider-files", {
      ...
      body: JSON.stringify({
        service: "imdb",
        files: { reviews: "PRIVATE-FILE-CONTENT" },
      }),
    });
    expect(unknown.status).toBe(400);
    ...
    const wrongFormat = await app.request("/v1/import/provider-files", {
      ...
      body: JSON.stringify({
        service: "imdb",
        files: { ratings: "PrivateHeader,Other\nPRIVATE-VALUE,VALUE" },
      }),
    });
    expect(wrongFormat.status).toBe(400);
    const wrongFormatBody = await wrongFormat.text();
    expect(wrongFormatBody).toContain(
      "IMDb ratings file must contain the required columns",
    );
...
describe("Letterboxd import-file export endpoint", () => {
  ...
  it("returns bounded user-controlled CSV files for a strict backup-v1 request", async () => {
    ...
    const body = (await response.json()) as {
      target: string;
      files: Array<{ content: string; recordCount: number }>;
    };
    ...
    expect(
      body.files.every(
        (file) =>
          new TextEncoder().encode(file.content).byteLength <= 1_000_000,
      ),
    ).toBe(true);
  });
  ...
  it("rejects unknown fields and conversions that would lose non-film state", async () => {
      ...
      body: JSON.stringify({
        backup: {
          ...
          ratings: [
            {
              ...backup.ratings[0],
              item: { ...backup.ratings[0].item, kind: "tv-show" },
            },
          ],
        },
        selection: { ratings: true },
    ...
    await expect(lossy.json()).resolves.toMatchObject({
      error: expect.stringContaining("only films"),
    });
  });
});
...
describe("API input validation", () => {
  it("rejects malformed JSON and oversized request bodies regardless of length metadata", async () => {
    ...
    const requestWithoutDeclaredLength = new Request(
      "http://localhost/v1/sync/plan",
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
    ...
    expect(requestWithoutDeclaredLength.headers.has("Content-Length")).toBe(
      false,
    );
    ...
    await expect(withoutDeclaredLength.json()).resolves.toEqual({
  ...
  it("validates rating conversion services, configured scales, and finite values", async () => {
    expect(
      (
        await app.request(
          "/v1/rating/convert?source=unknown&target=imdb&value=5",
        )
      ).status,
    ).toBe(400);
    expect(
      (
        await app.request(
          "/v1/rating/convert?source=tvmaze&target=imdb&value=5",
        )
      ).status,
    ).toBe(422);
    expect(
      (
        await app.request(
          "/v1/rating/convert?source=imdb&target=tmdb&value=not-a-number",
        )
      ).status,
    ).toBe(400);
  });
  ...
  it("rejects unknown request and connector-context fields instead of silently ignoring typos", async () => {
    ...
    const execution = await app.request("/v1/sync/execute", {
      ...
      body: JSON.stringify({
        ...
        target: "simkl",
        selection: { ratings: true },
        sourceContext: { accessTokn: "misspelled", apiKey: "key" },
        targetContext: {},
      }),
    });
    expect(execution.status).toBe(400);
    await expect(execution.json()).resolves.toMatchObject({
      error: expect.stringContaining("sourceContext"),
    });
    ...
    await expect(metadata.json()).resolves.toMatchObject({
      error: expect.stringContaining("strictly valid"),
    });
  });
});
...
describe("API access gate", () => {
  it("requires the configured server API key for versioned routes", async () => {
    ...
    expect(
      (
        await app.request("/v1/services", {
          headers: { Authorization: "Bearer test-server-key-with-extra-data" },
        })
      ).status,
    ).toBe(401);

    const authorized = await app.request("/v1/services", {
  ...
  it("exposes fixed-cardinality operational metrics only to an authenticated caller", async () => {
    const apiKey = "metrics-server-key";
    process.env.WATCHBRIDGE_API_KEY = apiKey;
    expect((await app.request("/v1/metrics")).status).toBe(401);
    expect(
      (
        await app.request("/v1/services", {
    ...
    expect(metrics.status).toBe(200);
    expect(metrics.headers.get("Content-Type")).toContain(
...
describe("Trakt device OAuth endpoints", () => {
  it("returns local device-flow validation failures as 400 without contacting Trakt", async () => {
    ...
    const started = await app.request("/v1/oauth/trakt/device/start", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ clientId: oversizedClientId }),
    });
    expect(started.status).toBe(400);
    await expect(started.json()).resolves.toEqual({
      error: "Trakt OAuth input is invalid.",
    });
...
describe("authorization-code OAuth endpoints", () => {
  ...
  it("maps typed OAuth failures without leaking unexpected native errors", () => {
    expect(
      oauthError(new OAuthInputError("invalid input"), "fallback"),
    ).toEqual({ error: "invalid input", status: 400 });
    expect(
      oauthError(new OAuthTransactionError("invalid state"), "fallback"),
    ).toEqual({ error: "invalid state", status: 400 });
    expect(
      oauthError(new OAuthCapacityError("at capacity"), "fallback"),
    ).toEqual({ error: "at capacity", status: 429 });
    expect(
      oauthError(new OAuthProviderError("Trakt", "http", 401), "fallback"),
    ).toEqual({
      error: "Trakt OAuth request failed (401).",
      status: 502,
    });
    expect(
      oauthError(
        new Error("native detail that must stay private"),
        "safe fallback",
      ),
    ).toEqual({
      error: "safe fallback",
  ...
  it("wraps synchronous OAuth starts and returns bounded-input failures as 400", async () => {
    ...
    const requests = [
      [
        "/v1/oauth/trakt/start",
        { clientId: oversized, redirectUri: "https://app.example/trakt" },
      ],
      [
        "/v1/oauth/myanimelist/start",
        { clientId: oversized, redirectUri: "https://app.example/mal" },
      ],
      [
        "/v1/oauth/shikimori/start",
        { clientId: oversized, redirectUri: "https://app.example/shikimori" },
      ],
      [
        "/v1/oauth/annict/start",
        { clientId: oversized, redirectUri: "urn:ietf:wg:oauth:2.0:oob" },
      ],
      [
        "/v1/oauth/simkl/start",
        { clientId: "simkl-client", userAgent: "x".repeat(513) },
      ],
    ] as const;
    ...
    for (const [path, body] of requests) {
      ...
      expect(response.status).toBe(400);
      await expect(response.json()).resolves.toMatchObject({
        error: expect.stringContaining("OAuth input is invalid"),
      });
    }
  });
  ...
  it("returns 429 at authorization capacity and accepts new starts after expiry", async () => {
    ...
    const replacement = await app.request("/v1/oauth/myanimelist/start", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ clientId: "replacement-client" }),
    });
  ...
  it("completes TMDb request-token exchange without returning the application credential", async () => {
    const remoteFetch = vi
      ...
      .mockResolvedValueOnce(
        Response.json({
          success: true,
          request_token: "tmdb-request-token",
          status_code: 1,
        }),
      )
      .mockResolvedValueOnce(
        Response.json({
          success: true,
          access_token: "tmdb-user-token",
          account_id: "tmdb-account-object",
          status_code: 1,
        }),
      );
    vi.stubGlobal("fetch", remoteFetch);
    const started = await app.request("/v1/oauth/tmdb/start", {
      ...
      body: JSON.stringify({
        ...
        redirectUri: "https://app.example/tmdb",
      }),
    });
    expect(started.status).toBe(200);
    const transaction = (await started.json()) as {
      state: string;
      authorizationUrl: string;
      applicationToken?: string;
    };
    expect(transaction.applicationToken).toBeUndefined();
    expect(transaction.authorizationUrl).toContain(
      "https://www.themoviedb.org/auth/access",
    );
    const exchanged = await app.request("/v1/oauth/tmdb/exchange", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ state: transaction.state }),
    });
    ...
    expect(String(remoteFetch.mock.calls[1]?.[1]?.body)).not.toContain(
      "tmdb-application-token",
    );
  });
  ...
  it("creates the v3 TMDb session and numeric account ID needed for writes", async () => {
    const remoteFetch = vi
      .fn()
      .mockResolvedValueOnce(
        Response.json({ success: true, session_id: "tmdb-session" }),
      )
    ...
    const response = await app.request("/v1/oauth/tmdb/session", {
      ...
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        applicationToken: "tmdb-app-token",
        userAccessToken: "tmdb-user-token",
      }),
    });
    expect(response.status).toBe(200);
    await expect(response.json()).resolves.toEqual({
      success: true,
      session_id: "tmdb-session",
      numeric_account_id: 42,
    });
  });
  ...
  it("rejects unsafe OAuth redirect URIs before contacting a provider", async () => {
    ...
    const response = await app.request("/v1/oauth/tmdb/start", {
      ...
      body: JSON.stringify({
        applicationToken: "tmdb-app-token",
        redirectUri: "https://user:password@app.example/callback",
      }),
    });
    expect(response.status).toBe(400);
    expect(remoteFetch).not.toHaveBeenCalled();
  ...
  it("starts and exchanges a state-verified Trakt authorization", async () => {
    ...
    const started = await app.request("/v1/oauth/trakt/start", {
      ...
      body: JSON.stringify({
        clientId: "client-id",
        redirectUri: "https://app.example/trakt",
      }),
    });
    expect(started.status).toBe(200);
    const transaction = (await started.json()) as {
  ...
  it("does not let another named tenant consume OAuth state", async () => {
    const aliceKey = "a".repeat(32);
    const bobKey = "b".repeat(32);
    process.env.WATCHBRIDGE_API_KEYS = JSON.stringify({
      alice: aliceKey,
      bob: bobKey,
    });
    const remoteFetch = vi.fn(async () => Response.json(completeTraktToken));
    ...
    const bobExchange = await app.request("/v1/oauth/trakt/exchange", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${bobKey}`,
      },
      body: JSON.stringify({ state, code: "bob-code", clientSecret: "secret" }),
    });
    expect(bobExchange.status).toBe(400);
    expect(remoteFetch).not.toHaveBeenCalled();

    const aliceExchange = await app.request("/v1/oauth/trakt/exchange", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${aliceKey}`,
    ...
    expect(aliceExchange.status).toBe(200);
    expect(remoteFetch).toHaveBeenCalledOnce();
  });
  ...
  it("starts and exchanges MyAnimeList without returning its PKCE verifier", async () => {
    ...
    const started = await app.request("/v1/oauth/myanimelist/start", {
      ...
      body: JSON.stringify({
        clientId: "mal-client",
        redirectUri: "https://app.example/mal",
      }),
    });
    const transaction = (await started.json()) as {
      state: string;
    ...
    expect(
      new URL(transaction.authorizationUrl).searchParams.get(
  ...
  it("starts, exchanges, and refreshes an exact-scope Shikimori authorization", async () => {
        ...
        return Response.json({
          access_token:
            form.grant_type === "refresh_token"
              ? "shikimori-replacement"
              : "shikimori-access",
          token_type: "Bearer",
          expires_in: 86_400,
          refresh_token:
            form.grant_type === "refresh_token"
              ? "shikimori-next-refresh"
              : "shikimori-refresh",
          scope: "user_rates",
        });
      },
    ...
    await expect(refreshed.json()).resolves.toMatchObject({
      access_token: "shikimori-replacement",
      refresh_token: "shikimori-next-refresh",
    });
  ...
  it("starts and exchanges a Simkl S256 PKCE authorization", async () => {
    ...
    const started = await app.request("/v1/oauth/simkl/start", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ clientId: "simkl-client" }),
    });
    const transaction = (await started.json()) as {
      state: string;
      authorizationUrl: string;
    ...
    const exchanged = await app.request("/v1/oauth/simkl/exchange", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ state: transaction.state, code: "simkl-code" }),
    });
    expect(exchanged.status).toBe(200);
    await expect(exchanged.json()).resolves.toMatchObject({
      access_token: "simkl-access",
  ...
  it("rejects unknown OAuth state as a client error without contacting a provider", async () => {
    ...
    const response = await app.request("/v1/oauth/simkl/exchange", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ state: "unknown-state", code: "code" }),
    });
    expect(response.status).toBe(400);
    await expect(response.json()).resolves.toMatchObject({
...
describe("metadata resolution endpoint", () => {
  it("accepts only paired, bounded, path-safe instance-scoped Emby IDs in canonical items", async () => {
    ...
    vi.stubGlobal("fetch", remoteFetch);
    const request = (externalIds: Record<string, unknown>) =>
      app.request("/v1/metadata/resolve", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          service: "tvmaze",
          item: {
            id: "emby:server-a:movie-a",
            kind: "movie",
            title: "Heat",
            externalIds,
          },
    ...
    expect(
      (await request({ emby: "movie-a", embyServer: "server-a" })).status,
    ).toBe(200);
    expect((await request({ emby: "movie-a" })).status).toBe(400);
    expect((await request({ emby: 1, embyServer: "server-a" })).status).toBe(
      400,
    );
    expect(
      (await request({ emby: "movie/a", embyServer: "server-a" })).status,
  ...
  it("accepts only paired positive IDs and canonical v4 scopes for Kodi items", async () => {
    ...
    const request = (
      ...
      kind: "movie" | "episode" | "tv-show" = "movie",
    ) =>
      app.request("/v1/metadata/resolve", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
    ...
    expect(
      (await request({ kodi: 42, kodiLibrary: scope.toUpperCase() })).status,
    ).toBe(400);
    expect(
      (
        await request({
          kodi: 42,
          kodiLibrary: "00000000-0000-1000-8000-000000000000",
        })
      ).status,
    ).toBe(400);
    expect(
      (await request({ kodi: 42, kodiLibrary: scope }, "tv-show")).status,
    ).toBe(400);
    expect(remoteFetch).toHaveBeenCalledOnce();
  });
  ...
  it("accepts only paired, path-safe server-scoped Plex rating keys and bounded GUIDs", async () => {
    ...
    expect(
      (await request({ plex: "library/metadata/42", plexServer: "server-a" }))
        .status,
    ).toBe(400);
    expect((await request({ plex: "42", plexServer: "server a" })).status).toBe(
      400,
    );
    ...
    expect(
      (
        await request({
          plex: "42",
          plexServer: "server-a",
          plexGuid: "plex://movie/bad id",
        })
      ).status,
    ).toBe(400);
    expect(
      (await request({ plex: "movie.42", plexServer: "server-a" })).status,
    ).toBe(400);
    expect((await request({ plex: "42", plexServer: "server.a" })).status).toBe(
      400,
    );
    expect(
      (
        await request({
          plex: "42",
          plexServer: "server-a",
          plexGuid: "plex://show/abc",
        })
      ).status,
    ).toBe(400);
    expect(
      (await request({ plex: "42", plexServer: "server-a" }, "anime")).status,
    ).toBe(400);
    expect(remoteFetch).toHaveBeenCalledOnce();
  });
  ...
  it("accepts exact positive Kitsu IDs only for documented metadata resource kinds", async () => {
    ...
    vi.stubGlobal("fetch", remoteFetch);
    const request = (
      kind: "anime" | "manga" | "episode" | "movie",
      kitsu: unknown,
    ) =>
      app.request("/v1/metadata/resolve", {
        method: "POST",
  ...
  it("resolves one exact Wikidata Q-item and rejects malformed IDs before a provider request", async () => {
    const remoteFetch = vi.fn(async (input: RequestInfo | URL) => {
      expect(String(input)).toBe(
        "https://www.wikidata.org/wiki/Special:EntityData/Q11424.json",
      );
      return Response.json({
        entities: {
          Q11424: {
  ...
  it("passes TMDb application credentials and versioned context fields to the connector", async () => {
    const remoteFetch = vi.fn(
      async (input: RequestInfo | URL, init?: RequestInit) => {
        expect(String(input)).toBe(
          "https://tmdb-v3.test/3/find/tt0113277?external_source=imdb_id",
        );
        expect(new Headers(init?.headers).get("Authorization")).toBe(
          "Bearer tmdb-application-token",
        );
        return Response.json({
          movie_results: [
    ...
    const response = await app.request("/v1/metadata/resolve", {
      ...
      body: JSON.stringify({
        ...
        context: {
          applicationToken: "tmdb-application-token",
          v3BaseUrl: "https://tmdb-v3.test/3",
        },
      }),
    });
    expect(response.status).toBe(200);
    await expect(response.json()).resolves.toMatchObject({
      matches: [{ externalIds: { tmdbMovie: 949 } }],
    });
  });
    ...
    try {
      const response = await app.request("/v1/metadata/resolve", {
        ...
        body: JSON.stringify({
          ...
          context: {
            applicationToken: "tmdb-application-token",
            v3BaseUrl: "https://attacker.example/steal",
          },
        }),
      });
      expect(response.status).toBe(400);
  ...
  it("permits a strict HTTPS provider URL with the exact owner opt-in", async () => {
    ...
    const remoteFetch = vi.fn(
      async (input: RequestInfo | URL, init?: RequestInit) => {
        expect(String(input)).toBe(
          "https://owner-proxy.example/tmdb/3/find/tt0113277?external_source=imdb_id",
        );
        expect(new Headers(init?.headers).get("Authorization")).toBe(
          "Bearer tmdb-application-token",
        );
    ...
    try {
      const response = await app.request("/v1/metadata/resolve", {
        ...
        body: JSON.stringify({
          ...
          context: {
            applicationToken: "tmdb-application-token",
            v3BaseUrl: "https://owner-proxy.example/tmdb/3",
          },
        }),
      });
      expect(response.status).toBe(200);
  ...
  it("uses TheTVDB only with request-scoped authorized credentials", async () => {
    const remoteFetch = vi.fn(async (input: RequestInfo | URL) => {
      if (String(input).endsWith("/login"))
        return Response.json({ data: { token: "tvdb-token" } });
      return Response.json({
        data: [
          {
            tvdb_id: 121361,
    ...
    const response = await app.request("/v1/metadata/resolve", {
      ...
      body: JSON.stringify({
        ...
        context: {
          ...
          baseUrl: "https://tvdb.test",
        },
      }),
    });
    expect(response.status).toBe(200);
    await expect(response.json()).resolves.toMatchObject({
      matches: [{ externalIds: { tvdb: 121361 } }],
    });
    expect(remoteFetch).toHaveBeenCalledTimes(2);
  });
});
  ...
  it("uses a request-scoped TasteDive context and forwards a bounded limit", async () => {
    const remoteFetch = vi.fn(
      async (input: RequestInfo | URL, init?: RequestInit) => {
        ...
        expect(`${url.origin}${url.pathname}`).toBe(
          "https://tastedive.test/api/similar",
        );
    ...
    const response = await app.request("/v1/recommendations", {
      ...
      body: JSON.stringify({
        ...
        context: {
          apiKey: "taste-key",
          baseUrl: "https://tastedive.test/api",
          userAgent: "WatchBridge Test/1.0",
        },
      }),
    });
      ...
      recommendations: [
        {
          ...
          referenceUrl: "https://example.test/thief",
        },
      ],
    });
    expect(remoteFetch).toHaveBeenCalledOnce();
  ...
  it("rejects malformed canonical items, unbounded limits, invalid contexts, and unsupported providers before fetch", async () => {
    ...
    expect(
      (
        await request({
          service: "tastedive",
          item: { ...item, externalIds: undefined },
          context: { apiKey: "key" },
        })
      ).status,
    ).toBe(400);
    ...
    expect(
      (await request({ service: "tastedive", item, context: [] })).status,
    ).toBe(400);
    expect(
      (
        await request({
          service: "tastedive",
          item,
          context: { apiKey: "key", accessToken: "not-used" },
        })
      ).status,
    ).toBe(400);
    expect(
      (
        await request({
          service: "tastedive",
          item,
          context: { apiKey: "key", userAgent: "bad\r\nheader" },
        })
      ).status,
    ).toBe(400);
    expect(
      (
        await request({
          service: "tastedive",
          item,
          context: { apiKey: "key", baseUrl: "http://tastedive.test/api" },
        })
      ).status,
    ).toBe(400);
    expect(
      (await request({ service: "tvmaze", item, context: { apiKey: "key" } }))
        .status,
    ).toBe(422);
    expect(remoteFetch).not.toHaveBeenCalled();
  });
...
describe("encrypted OAuth vault", () => {
  it("requires explicit confirmation/encryption, never returns context, and deletes a stored record", async () => {
    ...
    const context = {
      accessToken: "vault-secret-token",
      apiKey: "vault-client-id",
    };
    ...
    const metadata = (await stored.json()) as {
      id: string;
      service: string;
      accessToken?: string;
    };
    ...
    const deleted = await app.request(`/v1/oauth/vault/${metadata.id}`, {
      method: "DELETE",
    });
  ...
  it("isolates vault records by named API-key tenant", async () => {
    const directory = await mkdtemp(
      join(tmpdir(), "watchbridge-oauth-vault-tenants-"),
    );
    temporaryBackupDirectories.push(directory);
    const aliceKey = "a".repeat(32);
    const bobKey = "b".repeat(32);
    ...
    const context = {
      accessToken: "alice-vault-token",
      apiKey: "alice-client-id",
    };
    ...
    expect(await readdir(join(directory, "alice"))).toEqual([`${id}.json`]);

    // A copied ciphertext must not be usable by another tenant: its encryption
    // additional-authenticated data includes the original tenant ID.
    await mkdir(join(directory, "bob"));
    await writeFile(
      join(directory, "bob", `${id}.json`),
      await readFile(join(directory, "alice", `${id}.json`), "utf8"),
      "utf8",
    );

    const bobDelete = await app.request(`/v1/oauth/vault/${id}`, {
      method: "DELETE",
      headers: { Authorization: `Bearer ${bobKey}` },
    });
    expect(bobDelete.status).toBe(404);
    expect(await readdir(join(directory, "alice"))).toEqual([`${id}.json`]);
    expect(await readdir(join(directory, "bob"))).toEqual([`${id}.json`]);

    const aliceDelete = await app.request(`/v1/oauth/vault/${id}`, {
      method: "DELETE",
  ...
  it("uses a matching vault context for direct account sync without sending secrets again", async () => {
    const directory = await mkdtemp(
      join(tmpdir(), "watchbridge-oauth-vault-sync-"),
    );
    temporaryBackupDirectories.push(directory);
    process.env.WATCHBRIDGE_OAUTH_VAULT_DIR = directory;
    process.env.WATCHBRIDGE_STORAGE_KEY = "01".repeat(32);
    ...
    const sourceVaultId = await store("trakt", {
      accessToken: "source-vault-token",
      apiKey: "source-vault-key",
    });
    const targetVaultId = await store("simkl", {
      accessToken: "target-vault-token",
      apiKey: "target-vault-key",
    });
    const response = await app.request("/v1/sync/execute", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        ...
        selection: { ratings: true },
        dryRun: true,
        sourceContext: { vaultId: sourceVaultId },
        targetContext: { vaultId: targetVaultId },
      }),
    });
    expect(response.status).toBe(200);
    await expect(response.json()).resolves.toMatchObject({
      actions: [{ feature: "ratings", status: "skipped" }],
    });
    ...
    const wrongService = await app.request("/v1/sync/execute", {
      ...
      body: JSON.stringify({
        ...
        selection: { ratings: true },
        dryRun: true,
        sourceContext: { vaultId: targetVaultId },
        targetContext: { vaultId: targetVaultId },
      }),
    });
    expect(wrongService.status).toBe(400);
    await expect(wrongService.json()).resolves.toMatchObject({
      error: expect.stringContaining("Both sourceContext"),
    });
  });
});
...
describe("sync execution endpoint", () => {
  it("accepts an Emby HTTPS server only with the exact owner base-URL opt-in", async () => {
    ...
    process.env.WATCHBRIDGE_ALLOW_CUSTOM_PROVIDER_BASE_URLS = "true";
    const directory = await mkdtemp(
      join(tmpdir(), "watchbridge-emby-boundary-"),
    );
    temporaryBackupDirectories.push(directory);
    process.env.WATCHBRIDGE_JOB_DIR = directory;
    const remoteFetch = vi.fn(
      async (input: RequestInfo | URL, init?: RequestInit) => {
        ...
        if (url.pathname === "/root/System/Info")
          return Response.json({
            Id: "server-a",
            ServerName: "Emby",
            Version: "4.8.11.0",
          });
        if (url.pathname === "/root/Users/user-a")
          return Response.json({
            Id: "user-a",
            ServerId: "server-a",
            Name: "Sync User",
          });
        if (url.pathname === "/root/Users/user-a/Items")
    ...
    const body = {
      ...
      targetContext: {
        ...
        accountId: "user-a",
        baseUrl: "https://owner-emby.example/root",
      },
    };
    try {
      ...
      await expect(accepted.json()).resolves.toMatchObject({
        targetBackup: { service: "emby", watched: [] },
      });
      expect(remoteFetch).toHaveBeenCalledTimes(3);
  ...
  it("accepts a profile-scoped Kodi JSON-RPC endpoint only with strict context and owner opt-in", async () => {
    ...
    process.env.WATCHBRIDGE_ALLOW_CUSTOM_PROVIDER_BASE_URLS = "true";
    const directory = await mkdtemp(
      join(tmpdir(), "watchbridge-kodi-boundary-"),
    );
    temporaryBackupDirectories.push(directory);
    process.env.WATCHBRIDGE_JOB_DIR = directory;
    const remoteFetch = vi.fn(
      async (input: RequestInfo | URL, init?: RequestInit) => {
        ...
        expect(headers.get("Authorization")).toBe(
          "Basic a29kaS11c2VyOmtvZGktcGFzc3dvcmQ=",
        );
        ...
        const result = (() => {
          switch (request.method) {
            ...
            case "Application.GetProperties":
              return { name: "Kodi", version: { major: 21, minor: 0 } };
            case "Profiles.GetCurrentProfile":
              return { label: "Master user" };
            case "JSONRPC.Permission":
              return { readdata: true, updatedata: true };
            case "VideoLibrary.GetMovies":
              return { movies: [], limits: { start: 0, end: 0, total: 0 } };
            case "VideoLibrary.GetEpisodes":
              return { episodes: [], limits: { start: 0, end: 0, total: 0 } };
            default:
    ...
    const body = {
      ...
      targetContext: {
        ...
        baseUrl: "https://owner-kodi.example/jsonrpc",
      },
    };
    try {
      const accepted = await app.request("/v1/sync/from-backup", {
      ...
      expect(accepted.status).toBe(200);
      await expect(accepted.json()).resolves.toMatchObject({
        targetBackup: { service: "kodi", ratings: [] },
      });
  ...
  it("rejects account contexts that try to redirect tokens when URL overrides are disabled", async () => {
    ...
    process.env.WATCHBRIDGE_API_KEY = "production-api-key";
    const directory = await mkdtemp(
      join(tmpdir(), "watchbridge-provider-boundary-"),
    );
    temporaryBackupDirectories.push(directory);
    process.env.WATCHBRIDGE_JOB_DIR = directory;
    const remoteFetch = vi.fn();
      ...
      const response = await app.request("/v1/sync/execute", {
        ...
        body: JSON.stringify({
          ...
          sourceContext: {
            accessToken: "trakt-token",
            apiKey: "trakt-client",
            baseUrl: "https://attacker.example/trakt",
          },
          targetContext: {
            accessToken: "simkl-token",
            apiKey: "simkl-client",
            baseUrl: "https://attacker.example/simkl",
          },
        }),
      });
      expect(response.status).toBe(400);
  ...
  it("syncs a validated canonical backup from a manual service into an official target", async () => {
    ...
    const response = await app.request("/v1/sync/from-backup", {
      ...
      body: JSON.stringify({
          ...
          ratings: [
            {
              ...
              value: 8,
              scale: { min: 1, max: 10, step: 1, name: "Ten point" },
            },
          ],
          watched: [],
          watchlist: [],
        ...
        targetContext: {
          accessToken: "simkl-token",
          apiKey: "simkl-client",
          baseUrl: "https://simkl-backup.test",
        },
      }),
    });
    expect(response.status).toBe(200);
    await expect(response.json()).resolves.toMatchObject({
      ...
      actions: [
        { feature: "ratings", status: "previewed", count: 1, conflicts: 0 },
      ],
    });
  });
  ...
  it("persists bounded canonical conflict review without connector credentials", async () => {
    const directory = await mkdtemp(
      join(tmpdir(), "watchbridge-conflict-review-"),
    );
    temporaryBackupDirectories.push(directory);
    process.env.WATCHBRIDGE_JOB_DIR = directory;
    vi.stubGlobal(
            ...
            movies: [
              {
                status: "completed",
                user_rating: 7,
                user_rated_at: "2026-01-02T00:00:00Z",
                movie: {
                  title: "Heat",
                  year: 1995,
                  ids: { simkl: 1, imdb: "tt0113277", tmdb: "949" },
                },
              },
            ],
          });
    ...
    const response = await app.request("/v1/sync/from-backup", {
      ...
      body: JSON.stringify({
        ...
        conflictPolicy: "manual",
        targetContext: {
          accessToken: "target-private-token",
          apiKey: "target-private-key",
          baseUrl: "https://simkl-conflict.test",
        },
      }),
    });
    expect(response.status).toBe(200);
    ...
    expect(result.conflictDetails).toEqual([
      expect.objectContaining({
        feature: "ratings",
        direction: { source: "serializd", target: "simkl" },
    ...
    const stored = await readFile(
      join(directory, `${result.job.id}.json`),
      "utf8",
    );
  ...
  it("rejects durable jobs whose conflict summaries contain unknown or sensitive-shaped fields", async () => {
    const directory = await mkdtemp(
      join(tmpdir(), "watchbridge-invalid-conflict-review-"),
    );
    temporaryBackupDirectories.push(directory);
    process.env.WATCHBRIDGE_JOB_DIR = directory;
    const id = "11111111-1111-4111-8111-111111111111";
  ...
  it("rejects unversioned or internally inconsistent uploaded backups", async () => {
    const response = await app.request("/v1/sync/from-backup", {
      ...
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        backup: { service: "serializd", exportedAt: "2026-07-15T00:00:00Z" },
        target: "simkl",
        selection: { ratings: true },
      }),
    });
    expect(response.status).toBe(400);
    await expect(response.json()).resolves.toMatchObject({
      error: expect.stringContaining("watchbridge.backup.v1"),
    });
  ...
  it("runs a Trakt to TMDb dry-run with the exact v4 export and v3 write credentials", async () => {
    const remoteFetch = vi.fn(async (input: RequestInfo | URL) => {
      ...
      if (url.startsWith("https://tmdb-v4.test/4/"))
    ...
    const response = await app.request("/v1/sync/execute", {
      ...
      body: JSON.stringify({
        source: "trakt",
        target: "tmdb",
        selection: { watchlist: true },
        dryRun: true,
        sourceContext: {
          accessToken: "trakt-token",
          apiKey: "trakt-client",
          baseUrl: "https://trakt-context.test",
        },
        targetContext: {
          accessToken: "tmdb-user-token",
          applicationToken: "tmdb-application-token",
          accountObjectId: "tmdb-object-id",
          sessionId: "tmdb-session",
          numericAccountId: 42,
          v3BaseUrl: "https://tmdb-v3.test/3",
          v4BaseUrl: "https://tmdb-v4.test/4",
        },
      }),
    });
    expect(response.status).toBe(200);
  ...
  it("rejects services without a shipped official account connector", async () => {
    ...
    await expect(response.json()).resolves.toMatchObject({
      error: expect.stringContaining("implemented official API"),
    });
  });
  ...
  it("blocks an unsupported two-way account feature before provider access and audits it as retry-safe", async () => {
    const directory = await mkdtemp(
      join(tmpdir(), "watchbridge-blocked-two-way-"),
    );
    temporaryBackupDirectories.push(directory);
    process.env.WATCHBRIDGE_JOB_DIR = directory;
    ...
    expect(response.status).toBe(400);
    await expect(response.json()).resolves.toMatchObject({
      error: expect.stringContaining("Two-way sync is blocked"),
      retrySafe: true,
  ...
  it("rejects an unknown conflict policy before connecting to a service", async () => {
    const response = await app.request("/v1/sync/execute", {
      ...
      body: JSON.stringify({
        ...
        sourceContext: {},
        targetContext: {},
        conflictPolicy: "overwrite-everything",
      }),
    });
    expect(response.status).toBe(400);
    await expect(response.json()).resolves.toMatchObject({
      error: "Unknown conflictPolicy.",
    });
  });
  ...
  it("rejects malformed or duplicate per-record conflict choices before connecting", async () => {
    for (const conflictResolutions of [
      ...
      expect(response.status).toBe(400);
      await expect(response.json()).resolves.toMatchObject({
        error: expect.stringContaining("conflictResolutions"),
      });
    }
  });
  ...
  it("rejects malformed, duplicate, or unselected identity overrides before connecting", async () => {
    for (const identityOverrides of [
      [
        {
          feature: "ratings",
          sourceItemId: " movie:source",
          targetItemId: "movie:target",
        },
      ],
      [
        {
      ...
      expect(response.status).toBe(400);
      await expect(response.json()).resolves.toMatchObject({
        error: expect.stringContaining("identityOverrides"),
      });
    }
  });
  ...
  it("runs a dry-run through the official connector factory with request-scoped credentials", async () => {
    ...
    expect(result).toMatchObject({
      sourceBackup: { service: "trakt" },
      targetBackup: { service: "simkl" },
    ...
    await expect(
      (await app.request("/v1/sync/jobs")).json(),
    ).resolves.toMatchObject({ jobs: [{ id: result.job.id }] });
    expect(remoteFetch).toHaveBeenCalledTimes(13);
  });

  it("runs and durably audits a directional two-way dry run", async () => {
    const directory = await mkdtemp(
      join(tmpdir(), "watchbridge-two-way-jobs-"),
    );
    temporaryBackupDirectories.push(directory);
    process.env.WATCHBRIDGE_JOB_DIR = directory;
    vi.stubGlobal("fetch", vi.fn(emptyAccountExportFetch));
    ...
    const response = await app.request("/v1/sync/execute", {
      ...
      body: JSON.stringify({
        ...
        sourceContext: {
          accessToken: "source-token",
          apiKey: "source-key",
          baseUrl: "https://trakt-two-way.test",
        },
        targetContext: {
          accessToken: "target-token",
          apiKey: "target-key",
          baseUrl: "https://simkl-two-way.test",
        },
      }),
    });
    ...
    expect(result.actions).toEqual([
      ...
      expect.objectContaining({
        status: "skipped",
        direction: { source: "simkl", target: "trakt" },
      }),
    ]);
    await expect(
      (await app.request(`/v1/sync/jobs/${result.job.id}`)).json(),
  ...
  it("persists and serves the target backup before a confirmed execution", async () => {
    ...
    const backup = await app.request(
      `/v1/backups/${result.targetBackupArtifact.id}`,
    );
    ...
    const crossServiceRestore = await app.request(
      `/v1/backups/${result.targetBackupArtifact.id}/restore`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          target: "trakt",
          dryRun: true,
          targetContext: { accessToken: "token", apiKey: "key" },
        }),
      },
    );
    expect(crossServiceRestore.status).toBe(400);
    await expect(crossServiceRestore.json()).resolves.toMatchObject({
      error: expect.stringContaining("/v1/sync/from-backup"),
    });
  ...
  it("persists and audits both account snapshots before a confirmed two-way execution", async () => {
    ...
    const response = await app.request("/v1/sync/execute", {
      ...
      body: JSON.stringify({
        ...
        sourceContext: {
          accessToken: "source-token",
          apiKey: "source-key",
          baseUrl: "https://trakt-two-way-write.test",
        },
        targetContext: {
          accessToken: "target-token",
          apiKey: "target-key",
          baseUrl: "https://simkl-two-way-write.test",
        },
      }),
    });
    ...
    expect(result.sourceBackupArtifact.id).not.toBe(
      result.targetBackupArtifact.id,
    );
    await expect(
      (
        await app.request(`/v1/backups/${result.sourceBackupArtifact.id}`)
      ).json(),
    ).resolves.toMatchObject({ service: "trakt" });
    await expect(
      (
        await app.request(`/v1/backups/${result.targetBackupArtifact.id}`)
      ).json(),
    ).resolves.toMatchObject({ service: "simkl" });
    await expect(
      (await app.request(`/v1/sync/jobs/${result.job.id}`)).json(),
    ).resolves.toMatchObject({
      status: "succeeded",
  ...
  it("encrypts backup and job files at rest while returning their original API representations", async () => {
    const backupDirectory = await mkdtemp(
      join(tmpdir(), "watchbridge-encrypted-backup-"),
    );
    const jobDirectory = await mkdtemp(
      join(tmpdir(), "watchbridge-encrypted-job-"),
    );
    temporaryBackupDirectories.push(backupDirectory, jobDirectory);
    process.env.WATCHBRIDGE_BACKUP_DIR = backupDirectory;
    process.env.WATCHBRIDGE_JOB_DIR = jobDirectory;
    ...
    const response = await app.request("/v1/sync/execute", {
      ...
      body: JSON.stringify({
        ...
        sourceContext: {
          accessToken: "source-token",
          apiKey: "source-key",
          baseUrl: "https://trakt-encrypted.test",
        },
        targetContext: {
          accessToken: "target-token",
          apiKey: "target-key",
          baseUrl: "https://simkl-encrypted.test",
        },
      }),
    });
    expect(response.status).toBe(200);
  ...
  it("fails encrypted storage reads closed for a wrong or missing key and authenticated-data tampering", async () => {
    const backupDirectory = await mkdtemp(
      join(tmpdir(), "watchbridge-encrypted-reject-backup-"),
    );
    const jobDirectory = await mkdtemp(
      join(tmpdir(), "watchbridge-encrypted-reject-job-"),
    );
    temporaryBackupDirectories.push(backupDirectory, jobDirectory);
    process.env.WATCHBRIDGE_BACKUP_DIR = backupDirectory;
    process.env.WATCHBRIDGE_JOB_DIR = jobDirectory;
    ...
    const response = await app.request("/v1/sync/execute", {
      ...
      body: JSON.stringify({
        ...
        sourceContext: {
          accessToken: "source-token",
          apiKey: "source-key",
          baseUrl: "https://trakt-encrypted-reject.test",
        },
        targetContext: {
          accessToken: "target-token",
          apiKey: "target-key",
          baseUrl: "https://simkl-encrypted-reject.test",
        },
      }),
    });
    expect(response.status).toBe(200);
    ...
    for (const path of [backupPath, jobPath]) {
      ...
      await writeFile(
        ...
        JSON.stringify({
          ...envelope,
          tag: `${replacement}${envelope.tag.slice(1)}`,
        }),
      );
    }
    const backupFailure = await app.request(
      `/v1/backups/${result.targetBackupArtifact.id}`,
    );
    expect(backupFailure.status).toBe(404);
    await expect(backupFailure.json()).resolves.toEqual({
      error: "Unknown backup.",
    });
    const jobFailure = await app.request(`/v1/sync/jobs/${result.job.id}`);
    expect(jobFailure.status).toBe(404);
    await expect(jobFailure.json()).resolves.toEqual({
      error: "Unknown sync job.",
    });
  });
  ...
  it("rejects plaintext in encrypted mode unless one-time migration is explicitly enabled and completed", async () => {
    const backupDirectory = await mkdtemp(
      join(tmpdir(), "watchbridge-plaintext-backup-"),
    );
    const jobDirectory = await mkdtemp(
      join(tmpdir(), "watchbridge-plaintext-job-"),
    );
    temporaryBackupDirectories.push(backupDirectory, jobDirectory);
    process.env.WATCHBRIDGE_BACKUP_DIR = backupDirectory;
    process.env.WATCHBRIDGE_JOB_DIR = jobDirectory;
    ...
    expect(
      JSON.parse(
        await readFile(join(backupDirectory, `${backupId}.json`), "utf8"),
      ),
    ).toMatchObject({ schema: "watchbridge.backup.v1" });
    ...
    expect(
      (await readdir(backupDirectory)).every((name) => name.endsWith(".json")),
    ).toBe(true);
    expect(
      (await readdir(jobDirectory)).every((name) => name.endsWith(".json")),
    ).toBe(true);
  ...
  it("retains both durable backups, exact direction, and a failed job when a two-way provider write fails", async () => {
    const backupDirectory = await mkdtemp(
      join(tmpdir(), "watchbridge-failed-backup-"),
    );
    temporaryBackupDirectories.push(backupDirectory);
    process.env.WATCHBRIDGE_BACKUP_DIR = backupDirectory;
    const jobDirectory = await mkdtemp(
      join(tmpdir(), "watchbridge-failed-job-"),
    );
    temporaryBackupDirectories.push(jobDirectory);
    process.env.WATCHBRIDGE_JOB_DIR = jobDirectory;
    vi.stubGlobal(
      ...
      vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
            ...
            return Response.json([
              {
                ...
                movie: {
                  title: "Heat",
                  year: 1995,
                  ids: { trakt: 12, imdb: "tt0113277", tmdb: 949 },
                },
              },
            ]);
          }
        ...
        if (
          url.startsWith("https://simkl-failure.test/sync/ratings") &&
          init?.method === "POST"
        ) {
        ...
        if (url.includes("/users/settings"))
          return Response.json({ account: { type: "free" } });
        if (url.startsWith("https://simkl-failure.test/"))
          return Response.json({});
        throw new Error(`Unexpected test URL: ${url}`);
      }),
    );
    ...
    const response = await app.request("/v1/sync/execute", {
      ...
      body: JSON.stringify({
        ...
        sourceContext: {
          accessToken: "source-token",
          apiKey: "source-key",
          baseUrl: "https://trakt-failure.test",
        },
        targetContext: {
          accessToken: "target-token",
          apiKey: "target-key",
          baseUrl: "https://simkl-failure.test",
        },
      }),
    });
    ...
    const failure = (await response.json()) as {
      ...
      job: {
        ...
        failedDirection: { source: string; target: string };
    ...
    expect(failure).toMatchObject({
      ...
      job: {
        ...
        failedDirection: { source: "trakt", target: "simkl" },
        writeMayBePartial: true,
      },
    });
    expect(failure.sourceBackupArtifact.id).toBe(
      failure.job.sourceBackupArtifact.id,
    );
    expect(failure.targetBackupArtifact.id).toBe(
      failure.job.targetBackupArtifact.id,
    );
    ...
    const sourceBackup = await app.request(
      `/v1/backups/${failure.sourceBackupArtifact.id}`,
    );
    expect(sourceBackup.status).toBe(200);
    await expect(sourceBackup.json()).resolves.toMatchObject({
      schema: "watchbridge.backup.v1",
      service: "trakt",
    });
    const backup = await app.request(
      `/v1/backups/${failure.targetBackupArtifact.id}`,
    );
    expect(backup.status).toBe(200);
    await expect(backup.json()).resolves.toMatchObject({
      schema: "watchbridge.backup.v1",
  ...
  it("audits a failed restore with its new pre-restore backup and exact failed feature", async () => {
    const backupDirectory = await mkdtemp(
      join(tmpdir(), "watchbridge-restore-source-"),
    );
    temporaryBackupDirectories.push(backupDirectory);
    process.env.WATCHBRIDGE_BACKUP_DIR = backupDirectory;
    const jobDirectory = await mkdtemp(
      join(tmpdir(), "watchbridge-restore-job-"),
    );
    temporaryBackupDirectories.push(jobDirectory);
    process.env.WATCHBRIDGE_JOB_DIR = jobDirectory;
    const sourceBackupId = "11111111-1111-4111-8111-111111111111";
    await writeFile(
      join(backupDirectory, `${sourceBackupId}.json`),
      JSON.stringify({
        schema: "watchbridge.backup.v1",
        service: "simkl",
        exportedAt: "2026-01-01T00:00:00Z",
        ratings: [
          {
            item: {
              ...
              title: "Heat",
              year: 1995,
              externalIds: { simkl: 1, imdb: "tt0113277" },
            },
            sourceService: "simkl",
            value: 8,
            scale: { min: 1, max: 10, step: 1, name: "SIMKL 1-10" },
          },
        ],
        watched: [],
        watchlist: [],
    ...
    vi.stubGlobal(
      "fetch",
      vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
        if (String(input).includes("/sync/ratings") && init?.method === "POST")
          return new Response("", { status: 503 });
        if (String(input).includes("/users/settings"))
          return Response.json({ account: { type: "free" } });
        return Response.json({});
      }),
    );
    ...
    const response = await app.request(
      `/v1/backups/${sourceBackupId}/restore`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          ...
          targetContext: {
            accessToken: "token",
            apiKey: "key",
            baseUrl: "https://simkl-restore-failure.test",
          },
        }),
      },
    );
    ...
    await expect(
      (await app.request(`/v1/sync/jobs/${failure.job.id}`)).json(),
    ).resolves.toMatchObject({
      status: "failed",
      failedFeature: "ratings",
      writeMayBePartial: true,
    ...
    expect(
      (await app.request(`/v1/backups/${failure.targetBackupArtifact.id}`))
        .status,
    ).toBe(200);
  });
});
