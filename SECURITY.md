...
The API can encrypt newly persisted canonical backups and sync audit jobs with authenticated AES-256-GCM by setting `WATCHBRIDGE_STORAGE_KEY`. The key is optional so operators can rely on an encrypted host volume, but one of those controls should protect production storage. Encrypted mode rejects plaintext by default. Trusted legacy files can be admitted only during an explicit `WATCHBRIDGE_ALLOW_PLAINTEXT_STORAGE_MIGRATION=true` window; each record is strictly validated and atomically rewritten encrypted before it is returned. Disable that switch immediately after migration. Writes are flushed before rename and POSIX directory entries are flushed after rename, which protects against ordinary process crashes and improves power-loss durability; Windows uses its native rename semantics because it cannot flush a directory handle this way. See [Deployment](docs/DEPLOYMENT.md) for the exact key formats, migration procedure, authenticated context, rotation limitations, and metadata that remains visible.
