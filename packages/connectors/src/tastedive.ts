...
export class TasteDiveConnector implements WatchBridgeConnector {
  ...
  async recommend(item: CanonicalMediaItem, limit = 20): Promise<ConnectorRecommendation[]> {
    ...
    const response = await requestJson<TasteDiveResponse>(url, { headers: { Accept: 'application/json', 'User-Agent': this.ctx.userAgent } }, connectorHttpOptions('TasteDive', this.ctx));
