...
|---|---|
| `apps/web` | One-way/two-way planner and support percentages, six-provider OAuth helpers, thirteen-connector direct-account sync, dedicated/mapped file conversion, canonical-backup sync and restore, durable job review, authenticated pre-write backup downloads, Letterboxd target-file downloads, metadata resolution, and recommendations. |
...
TMDb, exact-IMDb-ID OMDb and Watchmode, exact-Q-item Wikidata, TVmaze, TheTVDB, and exact-ID Kitsu metadata resolution plus TasteDive recommendations are separate read-only paths. They do not enter the user-data sync executor and do not imply ratings, history, watchlist, review, or social support. OMDb and Watchmode use caller-provided API keys under their respective terms boundaries; Kitsu performs public unauthenticated anime/manga/episode resource-by-ID reads only. These metadata-only services contribute no account source or target slot.
...
- Request contexts cannot redirect provider connectors, metadata resolvers, or recommendations to custom `baseUrl`, `v3BaseUrl`, or `v4BaseUrl` values by default. Such a request is rejected before any provider fetch; the tightly constrained owner opt-in and SSRF implications are documented in [Deployment](DEPLOYMENT.md). Jellyfin, Emby, Kodi, and Movary require explicit owner-controlled HTTPS `baseUrl` values, so production use also requires that exact owner opt-in plus outbound network allowlisting. Movary additionally requires its `/api/` base path and an alphanumeric provider username. Plex instead discovers HTTPS connections from the authenticated Plex resources service and verifies the selected machine ID; outbound allowlisting still applies. Favorites/likes are not modeled as watchlist on Jellyfin or Emby; Emby registers only completed movie/exact-episode watched membership and blocks ratings.
