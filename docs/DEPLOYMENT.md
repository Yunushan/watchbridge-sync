...
Authenticated operators can scrape `GET /v1/metrics` in Prometheus text format. It reports process start time plus request counters and a bounded duration histogram grouped only by fixed endpoint category and status class; the histogram supports p95 latency alerting without unbounded labels. It never labels a metric with an API key, tenant, UUID, provider credential, or raw URL. Keep the endpoint on the private API network and configure the scraper with a WatchBridge API key. These are per-process metrics, so aggregate them at the monitoring system if a deployment later has more than one instance.
...
Keep the storage key in a secret manager, separate from the storage volume and its backups. A missing, malformed, or wrong key makes encrypted records unavailable without revealing whether authentication or decryption failed. For a controlled rotation, set the replacement in `WATCHBRIDGE_STORAGE_KEY` and the old key in `WATCHBRIDGE_STORAGE_KEY_PREVIOUS`; retained backup, job, and vault records that are read are authenticated with the previous key and atomically re-encrypted with the active key. Read every retained record (and verify the resulting envelopes) before removing the previous key. The previous-key setting without an active key fails readiness. Only one previous key is supported, so keep an escrow of every retired key until the retention window expires; losing a key before migration loses access to records that still require it. Encryption hides file contents, not directory names, UUID filenames, file sizes, or filesystem timestamps. Leave `WATCHBRIDGE_STORAGE_KEY` unset only when deployment policy deliberately relies on an encrypted host volume.
