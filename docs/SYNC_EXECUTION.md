...
`POST /v1/sync/execute` runs only the thirteen shipped direct-account connectors: TMDb, Trakt, Simkl, MyAnimeList, anime-only Shikimori, anime-only Annict, anime-only Bangumi, a user-selected Jellyfin server, a watched-only user-selected Emby server, one scoped Kodi library/profile, one selected Plex Media Server, a user-selected Movary `/api/` server/account, and AniList. AniList is exact-ID anime/manga media-list sync with reviews, guarded social-graph reads, and additive follow writes; it rejects timestamps, unsupported list states, and review shapes it cannot reproduce. Movary is movie-only and rejects replay/progress history or watchlist timestamps that it cannot round-trip. Their registered feature sets differ; file, metadata/recommendation, manual, and restricted workflows use separate paths.
...
`POST /v1/metadata/resolve` exposes metadata-only candidates for TMDb, OMDb, Watchmode, MDBList, Wikidata, TVmaze, TheTVDB, and Kitsu. OMDb and Watchmode require a request-scoped API key plus an exact IMDb title ID and are limited to their official HTTPS ID lookups under their provider terms boundaries. MDBList requires a request-scoped API key plus an exact positive TMDb movie ID and is limited to its documented movie route; it does not provide account sync through this path. Wikidata uses only its public HTTPS exact-Q-item entity-data route with an identifying User-Agent; it does not search, query SPARQL, edit entities, or access accounts. TheTVDB context must contain the authorized project `apiKey` and, when applicable, `subscriberPin`. Kitsu is unauthenticated and exact-ID only: the item must be anime, manga, or episode with a positive integer `externalIds.kitsu`; WatchBridge does not turn its title into search, Algolia, mapping, or account-library traffic.
